...
COST_LARGE = flags.DEFINE_float('cost_large', 100.0, 'The cost of large scale simulation in TPU hours')
COST_SMALL = flags.DEFINE_float('cost_small', 10.0, 'The cost of surrogate evaluation in TPU hours')
SAMPLE_FILE_LARGE = flags.DEFINE_string('sample_file_large', '', 'File containing large scale samples')
SAMPLE_FILE_SMALL = flags.DEFINE_string('sample_file_small', '', 'File containing small scale samples')
SAMPLE_FILE_SMALL_MANY = flags.DEFINE_string('sample_file_small_many', '', 'File containing many small scale samples')
OUTFILE = flags.DEFINE_string('outfile', '', 'File base for solving the results')
...
def remove_nans(x, y):
  nan_idcs_x = np.isnan(x)
  nan_idcs_y = np.isnan(y)
  idcs = []
  for i in range(len(x)):
    if nan_idcs_x[i] == False and nan_idcs_y[i] == False:
      idcs.append(i)
  idcs = np.stack(idcs)
...
def mfmc_params(x, y, budget_per_run_ls, budget_per_run_sm):
  x, y = remove_nans(x, y)
  sigma_1 = np.sqrt(np.var(x, ddof=1))
  sigma_21 = np.sqrt(np.var(y, ddof=1))
  rho12 = np.cov(np.stack((x, y)))[
      0, 1] / (sigma_1 * sigma_21)
...
def main(_):
  ...
  mc_large = np.asarray([mc_estimate(ls_results, budget, bpr_large_scale) for budget in budgets])
  mc_small = np.asarray([mc_estimate(sm_results, budget, bpr_small_scale) for budget in budgets])
  sigma_1 = np.var(ls_results)

  # mfmc
  cor_coeff, alpha, w, r = mfmc_params(ls_results, sm_results, bpr_large_scale, bpr_small_scale)
  mfmc_args = (ls_results, sm_results, sm_results_new, alpha, w, r)
  mfmc, n_ls, n_sm = np.asarray([mfmc_estimate(budget, *mfmc_args) for budget in budgets]).T
  max_budget=max_budget_valid(budgets, n_ls, n_sm, len(ls_results), len(sm_results_new))
  variance_reduction_ratio = (np.sqrt(1-cor_coeff**2)+np.sqrt(bpr_small_scale/bpr_large_scale*cor_coeff**2))**2
  mc_variance = sigma_1/n_ls
  mfmc_variance = mc_variance*variance_reduction_ratio
  np.savetxt(
      OUTFILE.value+'_mfmc_data.txt',
       np.stack((budgets, mc_large, mc_small, mfmc, n_ls, n_sm, mc_variance, mfmc_variance)).T,
       delimiter='\t',
  ...
  with open(OUTFILE.value+'_info.txt', 'w') as file:
    ...
    file.write(f'{alpha}\t')
  for var, name in zip([ls_results, sm_results, sm_results_new], ['ls', 'sm', 'sm_new']):
