...
DNN_LAYERS = flags.DEFINE_multi_integer('dnn_layers', [5,5,5], 'The layers of the DNN')
KEY_SEED = flags.DEFINE_integer('key_seed', 0, 'The DNN seed key')
TRAIN_DATA_FILE = flags.DEFINE_string('train_data', '', 'The file containing the train data')
TRAIN_TEST_SPLIT_IDX = flags.DEFINE_integer('train_test_split_idx', 900, 'Where to split data into train and validation')
...
def main(_):
  ...
  final_params, _ = dnopt.fit_dnn(init_params, (x_train, y_train), mse, (x_test, y_test))
  ...
  with open(OUTDIR.value+'dnn_bytes_'+OUTNAME.value, 'wb') as file:
    file.write(bb)
  np.savetxt(OUTDIR.value+'dnn_means_'+OUTNAME.value, np.array([*x_mean, *x_std, y_mean, y_std]))
  pass
